...
export async function derivePrfKey(
  options: PrfOptions = {},
): Promise<{ key: Uint8Array; credentialId: Uint8Array }> {
  ...
  console.log(
    "[WebAuthn PRF] Salt (first 8 bytes):",
  ...
  if (options.credentialId) {
    ...
    // ---------------------------------------------------------
    console.log(
      "[WebAuthn PRF] Starting Assertion (get) with credential ID...",
    );
    console.log(
      "[WebAuthn PRF] Credential ID (base64):",
      bytesToBase64(options.credentialId),
    );
    ...
    console.log(
      "[WebAuthn PRF] PRF extension (eval.first is ArrayBuffer):",
    ...
    try {
      ...
      console.log(
        "[WebAuthn PRF] Calling navigator.credentials.get with extensions...",
      );
      ...
      console.log(
        "[WebAuthn PRF] Assertion Extension Results:",
        JSON.stringify(results, null, 2),
      );
      console.log(
        "[WebAuthn PRF] Raw results object keys:",
        Object.keys(results),
      );

      const prfResult =
        (results as any).prf || (results as any)["extension:prf"];
      console.log("[WebAuthn PRF] PRF result object:", prfResult);

      if (!prfResult) {
        console.error(
          "[WebAuthn PRF] No PRF in results. Full results:",
          results,
        );
        throw new Error(
          "WebAuthn PRF: No prf extension in results. Your passkey provider may not support PRF. Google Password Manager has limited PRF support.",
        );
      }

      if (!prfResult.results) {
        throw new Error(
          "WebAuthn PRF: prf.results is missing. Authenticator may not support PRF.",
        );
      }

      if (!prfResult.results.first) {
        throw new Error(
          "WebAuthn PRF: prf.results.first is missing. PRF evaluation failed.",
        );
      }

      const key = new Uint8Array(prfResult.results.first);
      console.log(
        `[WebAuthn PRF] ✓ Key derived successfully (${key.length} bytes)`,
      );
      ...
      if (err.name === "NotAllowedError") {
        console.error(
          "[WebAuthn PRF] NotAllowedError during assertion - Possible causes:",
        );
    ...
    // ---------------------------------------------------------
    console.log(
      "[WebAuthn PRF] Starting Attestation (create) to register new credential...",
    );
    ...
    try {
      ...
      if (!credential) {
        throw new Error(
          "WebAuthn credential creation returned null (user canceled?)",
        );
      }
      ...
      console.log(
        "[WebAuthn PRF] Credential ID (base64):",
        bytesToBase64(credentialId),
      );

      const results = credential.getClientExtensionResults() as any;
      console.log(
        "[WebAuthn PRF] Attestation Extension Results:",
        JSON.stringify(results, null, 2),
      );
      ...
      if (prfResult) {
        ...
        if (prfResult.enabled === false) {
          console.error(
            "[WebAuthn PRF] PRF explicitly disabled by authenticator",
          );
          throw new Error(
            "WebAuthn PRF: Your device created a passkey but does not support PRF encryption. This feature requires a hardware security key or compatible platform authenticator.",
          );
        }
      }
      ...
      console.warn(
        "[WebAuthn PRF] PRF not evaluated during create, attempting follow-up get()...",
      );
      console.warn(
        "[WebAuthn PRF] Note: This may fail on mobile due to user gesture expiration",
      );
      ...
      if (err.name === "NotAllowedError") {
        ...
        console.error(
          "[WebAuthn PRF] InvalidStateError - Credential already exists for this user",
        );
      } else if (err.name === "SecurityError") {
        console.error(
          "[WebAuthn PRF] SecurityError - RP ID is not valid for this origin",
        );
      }
      throw err;
    }
...
export async function encryptWithPrfKey(
  ...
  try {
    ...
    console.log(
      `[WebAuthn PRF] ✓ Encrypted to ${result.ciphertext.length} base64 characters`,
    );
    return result;
  } catch (err) {
...
export async function decryptWithPrfKey(
  ...
  console.log(
    `[WebAuthn PRF] Decrypting ${ciphertext.length} base64 characters...`,
  );
  console.log(`[WebAuthn PRF] Key length: ${key.length} bytes`);
  ...
  try {
    ...
    console.log(
      `[WebAuthn PRF] IV: ${ivBytes.length} bytes, Ciphertext: ${ciphertextBytes.length} bytes`,
    );
