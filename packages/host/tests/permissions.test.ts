...
describe('isPathAllowed', () => {
    test('allows feed GET with public_read', () => {
        ...
        expect(isPathAllowed('/api/v1/feed', 'GET', permissions)).toBe(true);
    });

    test('denies feed POST without public_write', () => {
        const permissions = { permissions: ['public_read' as const] };
        expect(isPathAllowed('/api/v1/feed', 'POST', permissions)).toBe(false);
    });

    test('allows feed POST with public_write', () => {
        const permissions = { permissions: ['public_write' as const] };
        expect(isPathAllowed('/api/v1/feed', 'POST', permissions)).toBe(true);
    });

    test('allows user routes with public_read', () => {
        const permissions = { permissions: ['public_read' as const] };
        expect(isPathAllowed('/api/v1/user/me', 'GET', permissions)).toBe(true);
    });

    test('allows storage GET with storage_read', () => {
        const permissions = { permissions: ['storage_read' as const] };
        expect(isPathAllowed('/api/v1/storage/mykey', 'GET', permissions)).toBe(true);
    });

    test('denies storage PUT without storage_write', () => {
        const permissions = { permissions: ['storage_read' as const] };
        expect(isPathAllowed('/api/v1/storage/mykey', 'PUT', permissions)).toBe(false);
    });
    ...
    test('allows group routes with appropriate permissions', () => {
        ...
        expect(isPathAllowed('/api/v1/groups', 'GET', readPerms)).toBe(true);
        expect(isPathAllowed('/api/v1/groups/123/storage', 'PUT', writePerms)).toBe(true);
    });
    ...
    test('allows dm routes with appropriate permissions', () => {
        ...
        expect(isPathAllowed('/api/v1/dm/pubkey123', 'GET', readPerms)).toBe(true);
        expect(isPathAllowed('/api/v1/dm/pubkey123', 'POST', writePerms)).toBe(true);
    });
});
