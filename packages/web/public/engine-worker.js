(() => {
  ...
  async function postSpaceMessage(ctx, spaceId, body) {
    if (!ctx.currentPubkey)
      return { status: 401, body: { error: "Not authenticated" } };
    if (!body?.content)
  ...
  async function inviteMember(ctx, spaceId, body) {
    if (!ctx.currentPubkey)
      return { status: 401, body: { error: "Not authenticated" } };
    if (!body?.pubkey)
    ...
    await ctx.pool.publish(wrapper);
  ...
  async function getSpaceStore(ctx, spaceId) {
    if (!ctx.currentPubkey)
      return { status: 401, body: { error: "Not authenticated" } };
    const scopedId = `${ctx.appOrigin}:${spaceId}`;
    const keys = await getKeys(ctx);
  ...
  async function updateSpaceStore(ctx, spaceId, key, value) {
    if (!ctx.currentPubkey)
      return { status: 401, body: { error: "Not authenticated" } };
    const scopedId = `${ctx.appOrigin}:${spaceId}`;
    const keys = await getKeys(ctx);
...
`);
  ...
  self.onmessage = async (event) => {
    ...
    switch (message.type) {
      ...
      case "STREAM_OPEN":
        await poolReady;
        if (pool) {
          await handleStreamOpen(message, pool, currentPubkey);
        } else {
  ...
  async function matchRoute(method, fullPath) {
    ...
    if (storageMatch) {
      ...
      if (method === "PUT") {
        return {
          handler: async (body) => putStorage(storageCtx, key, body, { public: params.public }),
          params: { key }
        };
      }
      if (method === "DELETE") {
    ...
    if (spaceMatch) {
      ...
      if (method === "POST" && subPath === "/messages") {
        return {
          handler: async (body) => postSpaceMessage(spaceCtx, spaceId, body),
          params: { spaceId }
        };
      }
      if (method === "POST" && subPath === "/invite") {
        return {
          handler: async (body) => inviteMember(spaceCtx, spaceId, body),
          params: { spaceId }
        };
      }
    }
    ...
    const dmMatch = path.match(/^\/mirage\/v1\/dms\/([a-zA-Z0-9]+)$/);
    if (dmMatch) {
      ...
      if (method === "POST") {
        return {
          handler: async (body) => sendDM(dmCtx, targetPubkey, body),
          params: { pubkey: targetPubkey }
        };
      }
    }
