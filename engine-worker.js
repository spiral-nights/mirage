...
`);
 ...
 ` + inner);
  ...
  // src/engine/MiragePool.ts
  class MiragePool extends SimplePool {
    localRelay;
    constructor(localRelay) {
      super();
  ...
  async function loadSpaceKeys(ctx, storageId = KEY_STORAGE_ID) {
    try {
      const rawMap = await internalGetStorage(ctx, storageId);
      if (!rawMap) {
        return new Map;
      }
      const map = new Map;
  ...
  async function saveSpaceKeys(ctx, keys, storageId = KEY_STORAGE_ID) {
    try {
      const rawMap = {};
  ...
  class SpaceService {
    ...
    async createSpace(name, forAppOrigin, offline = false) {
      if (!this.ctx.currentPubkey)
        throw new Error("Not authenticated");
    ...
    async saveKeys(keys) {
      const onlineKeys = new Map;
      const offlineKeys = new Map;
  ...
  function matchFilters2(filters, event) {
    for (let i2 = 0;i2 < filters.length; i2++) {
      if (matchFilter2(filters[i2], event)) {
        return true;
      }
    }
    return false;
    ...
    close(reason = "closed by caller") {
      if (!this.closed && this.relay.connected) {
        try {
          ...
          if (err instanceof SendingOnClosedConnection2) {} else {
            throw err;
          }
        }
        this.closed = true;
  ...
  // ../../node_modules/nostr-tools/lib/esm/filter.js
  ...
  function matchFilters3(filters, event) {
    for (let i2 = 0;i2 < filters.length; i2++) {
      if (matchFilter3(filters[i2], event)) {
        return true;
      }
    }
    return false;
  ...
  // src/engine/LocalRelay.ts
  class LocalEventStore extends import_wrapper_default {
    events;
    constructor() {
  ...
  class LocalRelay extends AbstractRelay2 {
    db;
    ...
    async publish(event) {
      ...
      if (event.kind === 0 || event.kind === 3 || event.kind >= 1e4 && event.kind < 20000 || event.kind >= 30000 && event.kind < 40000) {
        await this.handleReplaceable(event);
      }
      await this.db.events.put(event);
      for (const [, sub] of this.localSubs) {
        if (matchFilters3(sub.filters, event)) {
          sub.onevent(event);
        }
      }
      return "ok";
    }
    async handleDeletion(deleteEvent) {
      ...
      if (eventIdsToDelete.length === 0)
        return;
      await this.db.events.where("id").anyOf(eventIdsToDelete).filter((event) => event.pubkey === deleteEvent.pubkey).delete();
    ...
    async handleReplaceable(event) {
      ...
      let collection = this.db.events.where("kind").equals(event.kind).filter((e) => e.pubkey === event.pubkey);
    ...
    async queryFilter(filter) {
      ...
      if (filter.limit && filter.limit > 0) {
        results = results.sort((a, b) => b.created_at - a.created_at).slice(0, filter.limit);
      }
      return results;
    }
    async deleteByDTagPrefix(prefix) {
      const eventsToDelete = await this.db.events.filter((event) => {
  ...
  class LocalSubscription {
    ...
    async fire() {
      try {
        ...
        this.close(`query error: ${err.message}`);
      }
    }
    close(reason = "closed by caller") {
      if (this.closed)
  ...
  var localRelayInstance = null;
  function getLocalRelay() {
...
`);
  ...
  // src/engine/index.ts
  var localRelay = getLocalRelay();
  var pool = new MiragePool(localRelay);
  var activeRelays = [];
