...
var previewStorage = new Map;
var previewContacts = [];
var previewDMs = new Map;
var previewEvents = [];
var previewLibrary = [];
var PREVIEW_PUBKEY = "preview" + Array.from({ length: 60 }, () => Math.floor(Math.random() * 16).toString(16)).join("");
var PREVIEW_PROFILE = {
...
async function handlePreviewGet(path) {
  ...
  if (path === "/mirage/v1/space") {
    if (currentSpace)
      return jsonResponse(currentSpace);
...
async function handlePreviewPut(path, body) {
  ...
  if (storeKeyMatch) {
    ...
    return jsonResponse({ key, value: body, updatedAt: now2() });
  }
  return jsonResponse({ success: true });
}
...
// src/bridge/messaging.ts
var enginePort = null;
var isChildMode = false;
var engineReadyResolve;
var privateKeyBytes = null;
var publicKey = null;
var currentAppOrigin = null;
function hexToBytes4(hex2) {
  let bytes4 = new Uint8Array(hex2.length / 2);
  for (let i2 = 0;i2 < hex2.length; i2 += 2) {
    bytes4[i2 / 2] = parseInt(hex2.substring(i2, i2 + 2), 16);
  }
  return bytes4;
}
function setPrivateKey(key) {
  try {
    if (key) {
      privateKeyBytes = hexToBytes4(key);
...
function setEnginePort(port) {
  enginePort = port;
}
function setChildMode(value) {
  isChildMode = value;
}
function signalEngineReady() {
  engineReadyResolve();
}
function handleEngineMessage(event) {
  ...
  if (message.type === "API_RESPONSE") {
    ...
    handleDecryptRequest(message);
  } else if (message.type === "ACTION_SIGN_EVENT") {
    handleSignRequest(message);
  } else if (message.type === "SET_APP_ORIGIN") {
    currentAppOrigin = message.origin;
  } else if (message.type === "SET_SPACE_CONTEXT") {
    const ctxMsg = message;
    setPreviewSpaceContext(ctxMsg.spaceId, ctxMsg.spaceName);
  }
}
async function handleEncryptRequest(message) {
  try {
    if (privateKeyBytes) {
      const conversationKey = nip44_exports.v2.utils.getConversationKey(privateKeyBytes, message.pubkey);
      const ciphertext2 = nip44_exports.v2.encrypt(message.plaintext, conversationKey);
      postToEngine({ type: "ENCRYPT_RESULT", id: message.id, ciphertext: ciphertext2 });
      return;
    }
...
async function handleDecryptRequest(message) {
  try {
    if (privateKeyBytes) {
      ...
      const conversationKey = nip44_exports.v2.utils.getConversationKey(privateKeyBytes, message.pubkey);
      const plaintext2 = nip44_exports.v2.decrypt(message.ciphertext, conversationKey);
      postToEngine({ type: "DECRYPT_RESULT", id: message.id, plaintext: plaintext2 });
      return;
    }
    const nostr = window.nostr;
    if (!nostr) {
...
async function handleSignRequest(message) {
  try {
    if (privateKeyBytes) {
      const signedEvent2 = finalizeEvent(message.event, privateKeyBytes);
      postToEngine({ type: "SIGNATURE_RESULT", id: message.id, signedEvent: signedEvent2 });
      return;
    }
    const nostr = window.nostr;
    if (!nostr) {
...
// src/bridge/fetch.ts
async function interceptedFetch(rawInput, init) {
...
function handleStreamingRequest(url, init) {
  const id = crypto.randomUUID();
  const { readable, writable } = new TransformStream;
  const writer = writable.getWriter();
  const controller = {
    enqueue: (chunk) => writer.write(chunk),
    close: () => writer.close(),
...
// src/bridge/event-source.ts
class MirageEventSource extends EventTarget {
  readyState = 0;
  url;
...
async function initBridge(options = {}) {
  ...
  if (window.parent !== window) {
    ...
    if (!options.workerUrl)
      throw new Error("Worker URL required for Standalone Mode");
    ...
    if (options.privateKey) {
      ...
      if (!effectivePubkey) {
        try {
          const bytes4 = new Uint8Array(options.privateKey.match(/.{1,2}/g).map((byte) => parseInt(byte, 16)));
          effectivePubkey = getPublicKey(bytes4);
        } catch (e) {
    ...
    if (effectivePubkey) {
      ...
      if (hexKey.startsWith("npub")) {
        try {
          const decoded = nip19_exports.decode(hexKey);
  ...
  window.EventSource = MirageEventSource;
  if (typeof window.__mirageBridgeReady === "function") {
    window.__mirageBridgeReady(interceptedFetch);
  } else {
